...
createJsonFile(
  "../assets/data/nav_links/nav_projects.json",
  "../pages/projects"
);
...
function dirList(path) {
  let onlyPath = path.slice(3, path.lastIndexOf("/"));
...
/**
 * Checks if an image is saved in the "puplic/icons" directory
 ...
 * returns "../public/icons/"+LAST_DIR_NAME+EXTENSION if found, can be used to
...
function linkHref(path) {
  // let onlyFile = path.slice(path.lastIndexOf("/"), path.length - 3);
  let onlyFile = path.slice(8, path.length - 3);
  // if file name is "index.js"
  if (path.slice(path.length - 9, path.length - 3) === "/index") {
    onlyFile = path.slice(8, path.length - 9);
    let fromDir = dirList(path)[dirList(path).length - 1];
    if (fromDir === "pages") {
      return "/";
    }
...
function navLinkObject(fileArray) {
  ...
  for (let i = 0; i < fileArray.length; i++) {
    let tempArray = fileArray[i].path.substring(3);
 ...
 * @param {string} inDirectoryPath path of directory which will be processed, needs to start with "../"
 */
export default function createJsonFile(outJsonFilePath, inDirectoryPath) {
  ...
  let errMessageFile = outJsonFilePath.slice(
    outJsonFilePath.lastIndexOf("/") + 1,
    outJsonFilePath.length
  );
